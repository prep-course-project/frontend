...
export default function Sell_features({ objPost }) {
  ...
  const featureClick = (e) => {
    ...
    if (e.target.classList[1] === "active") {
      ...
      setAddFeature((current) =>
        current.filter((ele) => ele !== e.target.innerHTML)
      );
      // objPost(current => ({...current , features:addFeature}))
    }
  };
  ...
  const closeClick = (e) => {
    ...
    setAddFeature((current) =>
      current.filter((ele) => ele !== e.target.nextElementSibling.innerHTML)
    );
    let ele = [...document.querySelectorAll(".active")];

    ele.forEach(
      (item) =>
        item.innerText === e.target.nextElementSibling.innerHTML &&
  ...
  return (
    <div className="sell-feature">
      ...
      <button onClick={handile} className="add-feature-btn">
        Next
      </button>
    </div>
  );
